report 83001 "Lease Contract Sales Invoice"
{
    RDLCLayout = './Layout/Rep83001.LeaseContractSalesInvoice.rdlc';
    WordLayout = './Report/Rep83001.LeaseContractSalesInvoice.docx';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(CompanyBuildingName; CompanyInfo."Building Name")
            {
            }
            ...
            column(CompanyAddressformat; g_CompanyAddress)
            {
            }
            column(BankAccountName; g_BankAcctName)
            {
            }

            column(BankCode; g_BankCode)
            {
            }

            column(BankAddress; g_BankAcctAddress)
            {
            }
            column(BankAddress2; g_BankAcctAddress2)
            {
            }
            column(BankPostCode; g_BankAcctPostCode)
            {
            }

            column(BankCity; g_BankAcctCity)
            {
            }

            column(BankCountry; g_BankAcctCountry)
            {
            }
            ...
            column(CompanyBankAccountPostingGroup; g_BankAcctPostingGroup)
            {
            }
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(Invoice_Description_Line; "Invoice Description")
                {
                }
                ...
                column(NetLbl; NetLbl)
                {
                }


                column(PaidLbl; PaidLbl)
                {
                }
                ...
                column(ItemType_Line_Lbl; ItemType_Line_Lbl)
                {
                }
                ...
                // {
                // }
                column(VATPct_Line; Line."VAT %" / 100)
                {
                ...
                column(TotalUnitPrice; round(TotalUnitPrice, 0.01))
                {
                }
                column(VatAmount; g_VatAmount)
                {
                    AutoFormatExpression = 'USD';
                    AutoFormatType = 1;
                }
                ...
                column(BookingcontactLbl; BookingcontactLbl)
                {
                }
                ...
                begin
                    ...
                    If (type = Line.Type::"G/L Account") And ("No. of Days to Bill" <> 0) then begin
                        ...
                        //         begin
                        //             g_calcbillingdays := Abs(Line."Lease To Date" - Line."Lease From Date");
                        //             g_rate := LeaseContractHeader."Daily Rent";

                        //         end;
                        //     LeaseContractHeader."Payment Type"::Monthly:
                        //         begin
                        //             g_calcbillingdays := Abs(Line."Lease To Date" - Line."Lease From Date") + 1;
                        //             g_rate := LeaseContractHeader."Monthly Rent";
                        //         end;

                        // end;
                        g_calcbillingdays := "No. of Days to Bill";
                        //g_calcbillingdaysTxt := format(g_calcbillingdays) + ' Days';
                        g_calcbillingdaysTxt := '-';
                    end else begin
                        g_calcbillingdays := Quantity;
                        g_calcbillingdaysTxt := format(g_calcbillingdays);
                        g_rate := Round("Line Amount", 0.01);
                    end;

                    If Line."Unit Price" <> 0 then
                        TotalUnitPrice += Line."Unit Price";
                    If g_calcbillingdays = 0 then
                        g_calcbillingdaysTxt := '-';
                    ...
                    g_VatAmount := "Amount Including VAT" - "VAT Base Amount";
                    VATAmountLine.Init();
            ...
            dataitem(Totals; "Integer")
            {
                ...
                // column(TotalSubTotalFormat; Format(TotalSubTotal, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, Header."Currency Code")))
                // {
                // }
            ...
            begin
                ...
                g_CompanyAddress := CompanyAddr[3];
                If CompanyAddr[4] <> '' then
                    g_CompanyAddress := StrSubstNo('%1, %2', g_CompanyAddress, CompanyAddr[4]);
                If CompanyAddr[5] <> '' then
                    g_CompanyAddress := StrSubstNo('%1, %2', g_CompanyAddress, CompanyAddr[5]);
                If CompanyAddr[6] <> '' then
                    g_CompanyAddress := StrSubstNo('%1, %2', g_CompanyAddress, CompanyAddr[6]);
    ...
    requestpage
    {
        ...
        begin
            ...
            Clear(g_calcbillingdays);
            Clear(g_paymenttype);
            Clear(g_VatAmount);
            TotalExclVATText := 'Total';
            TotalInclVATText := 'Total';
            TotalText := 'Total';
        end;
    ...
    begin
        ...
        Sequence := 0;
        GetCompanyBankAccountDetail(CompanyInfo);
    ...
    var
        ...
        UnitLbl: Label 'Unit';
        ItemNo_Line_Lbl: Label 'Ref';
        ItemType_Line_Lbl: Label 'Item';
        VATClausesText: Text;
        QtyLbl: Label 'Qty', Comment = 'Short form of Quantity';
        PricePerLbl: Label 'Price per';
        CurrCode: Text[10];
        CurrSymbol: Text[10];
        ReportLbl: Label 'Tax Invoice';
        Sequence: Integer;
        g_BankAcctPostingGroup: Code[20];

        g_BankCode: Text[20];
        g_BankAcctName: Text[100];

        g_BankAcctAccountNo: Text[30];
        g_BankAcctAddress: Text[250];
        g_BankAcctAddress2: Text[50];
        g_CompanyAddress: Text[250];
        g_BankAcctPostCode: Text[30];
        g_BankAcctCity: Text[30];
        g_BankAcctCountry: Text[10];
        PaymentTermLbl: Label 'Payment Terms subjected to Licence Agreement';
        YourReferenceLbl: Label 'Your Reference';
        CompanyVATRegNo_Lbl: Label 'GST Reg No.:';
        FromToLbl: Label 'From - To';
        RateLbl: Label 'Rate';
        NetLbl: Label 'Net';
        VATLbl: Label 'GST (%)';
        PriceLbl: Label 'Price';
        PaidLbl: Label 'Paid';
        BookingRefLbl: Label 'Booking reference';
        g_Customer_Name: Text[100];
        BookingcontactLbl: Label 'Booking Contact:';
        g_calcbillingdays: Integer;
        g_rate: Decimal;
        g_calcbillingdaysTxt: Text;
        LeaseContractSetup: Record "Lease Contract Setup";

        TotalUnitPrice: Decimal;

        g_paymenttype: Option;
    ...
    procedure GetCompanyBankAccountDetail(CompanyInfo: Record "Company Information")
    var
        BankAccount: Record "Bank Account";
    begin
        Clear(g_BankAcctPostingGroup);
        Clear(g_BankAcctName);
        Clear(g_BankAcctAccountNo);
        Clear(g_BankAcctPostCode);
        Clear(g_BankAcctAddress);
        Clear(g_BankAcctAddress2);
        Clear(g_BankAcctCity);
        Clear(g_BankAcctCountry);
        BankAccount.SetRange("Bank Account No.", CompanyInfo."Bank Account No.");
        If BankAccount.FindFirst() then begin
            g_BankCode := BankAccount."Bank Code";
            g_BankAcctName := BankAccount.Name;
            g_BankAcctAccountNo := BankAccount."Bank Account No.";


            g_BankAcctAddress := BankAccount."Address";
            If BankAccount."Address 2" <> '' then
                g_BankAcctAddress := StrSubstNo('%1, %2', g_BankAcctAddress, BankAccount."Address 2");
            If BankAccount.City <> '' then
                g_BankAcctAddress := StrSubstNo('%1, %2', g_BankAcctAddress, BankAccount.City);
            If BankAccount."Country/Region Code" <> '' then
                g_BankAcctAddress := StrSubstNo('%1, %2', g_BankAcctAddress, BankAccount."Country/Region Code");
            If BankAccount."Post Code" <> '' then
                g_BankAcctAddress := StrSubstNo('%1, %2', g_BankAcctAddress, BankAccount."Post Code");
        end;
    end;


    //Get Bank Account Posting Group <<
