report 83010 "Con. Aged Account Payable"

{
    DefaultLayout = RDLC;
    RDLCLayout = './Layout/Rep83003.ConAgedAccountsPayable.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'Consolidated Aged Payable Report';
        ...
        Dataitem(Company; Company)
        {
            ...
            dataitem(Vendor; Vendor)
            {
                ...
                dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
                {
                    ...
                    begin
                        VendorLedgEntry.ChangeCompany(Company.Name);
            ...
            begin
                If g_Company <> '' then
                    if StrPos(g_Company, Company.Name) = 0 then CurrReport.Skip();
                Vendor.ChangeCompany(Company.Name);
                "Vendor Ledger Entry".ChangeCompany(Name);
                VendorLedgEntryEndingDate.ChangeCompany(Name);
                TempCurrency.ChangeCompany(Name);
                TempCurrency2.ChangeCompany(Name);
                Clear(TempCurrencyAmount);
                TempCurrencyAmount.ChangeCompany(Name);
                DetailedVendorLedgerEntry.ChangeCompany(Name);
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                group(Filter)
                {
                    ...
                    field(Company_Filter; g_Company)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Company';
                        ToolTip = 'Specifies the companies that you want the report to show.';


                        trigger OnDrillDown()
                        var
                        begin
                            DrillDownCompany(g_Company);
                        end;
        ...
        begin
            ...
            //g_Company := 'Jalan Sultan Investment 1 Ltd.|Z_Testing Company (Jalan)';
        end;
    ...
    local procedure DrillDownCompany(Var CompanyFilter: Text[2048])
    var
        l_pageCompany: page Companies;
        l_recCompany: Record Company;

        l_txtFilter: Text[2048];
        l_recCompanyInfo: Record "Company Information";
    begin
        l_recCompanyInfo.reset;
        l_recCompany.reset;
        l_pageCompany.LOOKUPMODE := TRUE;
        l_pageCompany.EDITABLE := FALSE;

        l_txtFilter := '<>' + CompanyFilter;

        WHILE STRPOS(l_txtFilter, '|') > 0 DO
            l_txtFilter := DELSTR(l_txtFilter, STRPOS(l_txtFilter, '|')) + '&<>' + COPYSTR(l_txtFilter, STRPOS(l_txtFilter, '|') + STRLEN('|'));//Replace | to &<>


        /*l_recCompany.FILTERGROUP(2);
        IF CompanyFilter <> '' THEN
            l_recCompany.SETFILTER(Name, l_txtFilter);
        l_recCompany.FILTERGROUP(0);*/

        l_pageCompany.SETTABLEVIEW(l_recCompany);
        IF l_pageCompany.RUNMODAL = ACTION::LookupOK THEN BEGIN
            l_pageCompany.GetRecords(l_recCompany);
            l_recCompany.MARKEDONLY := TRUE;
            IF l_recCompany.FINDSET THEN
                REPEAT
                    IF CompanyFilter = '' THEN
                        CompanyFilter += l_recCompany.Name
                    ELSE
                        CompanyFilter += '|' + l_recCompany.Name;
